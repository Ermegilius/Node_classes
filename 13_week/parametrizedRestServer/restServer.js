...
function startServer(config) {
	...
	app.get(`/api/${storage.RESOURCE}/primarykey`, (req, res) => res.json(storage.PRIMARY_KEY)); //returns PRIMARY_KEY

	app.get(`/api/${storage.RESOURCE}`, (req, res) => storage.getAll().then((result) => res.json(result)));

	app.get(`/api/${storage.RESOURCE}/keys`, (req, res) => storage.KEYS.then((result) => res.json(result))); //returns all keys
	...
	// app.post(`/api/${storage.RESOURCE}/:key/:value`, (req, res) =>
	// 	storage.get(req.params.value, req.params.key).then((result) => res.json(result))
	// );
