...
const PRIMARY_KEY = "id"; //hardcoded, not good
...
async function updateStorage(modifiedObjext) {
    const storage = await readStorage(storageFilePath);
    const oldObject = storage.find(
        (item) => item[PRIMARY_KEY] == modifiedObjext[PRIMARY_KEY]
    );
    if (oldObject) {
        Object.assign(oldObject, adapt(modifiedObjext));
        return await writeStorage(storageFilePath, storage);
    }
    return false;
}
